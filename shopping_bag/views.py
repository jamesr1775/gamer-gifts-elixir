    ...
    if size:
        if product_id in shopping_bag.keys():
            if size in shopping_bag[product_id]['item_with_sizes'].keys():
                if shopping_bag[product_id]['item_with_sizes'][size] + product_quantity > settings.MAX_ORDER_QUANTITY:
                    messages.error(request, f'Failed to add {product.name} to your bag, Maximum quantity is {settings.MAX_ORDER_QUANTITY}')
